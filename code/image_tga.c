...
/* load header manually because not all of these values are aligned */
	...
	{
		if (out_error)
			*out_error = ("tga: bad origin (must be top left or bottom left)");
		return NULL;
	}
...
/* if bit 5 of attributes isn't set, the image has been stored from bottom to top */
	...
	case 1:
		...
		{
			if (out_error)
				*out_error = ("tga: bad pixel_size");
			return NULL;
		}
		if (colormap_length != 256)
		{
			if (out_error)
				*out_error = ("tga: bad colormap_length");
			return NULL;
		}
		if (colormap_index != 0)
		{
			if (out_error)
				*out_error = ("tga: bad colormap_index");
			return NULL;
		}
		...
		default:
			if (out_error)
				*out_error = ("tga: bad colormap_size");
			return NULL;
		case 24:
